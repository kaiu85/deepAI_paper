...
# Parameters
n_s = 10 # States
base_name = 'supplement_only_proprioceptive_action' # Name for Saves and Logfile
...
action_params.append(Wa_aht_st)
...
action_params.append(Wa_atmu_aht)
...
action_params.append(Wa_atsig_aht)
...
action_sigmas = initialize_sigmas(action_params, init_sig_perturbations)
    
[r_params, r_epsilons] = randomize_parameters( params, sigmas, sig_min_perturbations )

[r_action_params, r_action_epsilons] = randomize_parameters( action_params, action_sigmas, sig_min_perturbations )
...
r_Wa_atsig_aht, r_ba_atsig] = r_action_params
...
r_Wa_atsig_aht, r_ba_atsig\
    ...
    # Only Proprioceptive Action
    FEta = p_oat
    
    return st, post, vt, oat, ot, oht, FEt, KL_st, hst, hst2, stmu, stsig, force, p_ot, p_oht, p_oat, FEta
...
((states_th, pos_th, vt_th, oat_th, ot_th, oht_th, FEt_th, KL_st_th, hst_th, hst2_th, stmu_th, stsig_th, force_th, p_ot_th, p_oht_th, p_oat_th, FEta_th), fe_updates) =\
                     theano.scan(fn=inner_fn,
                     sequences = [numpy.arange(n_run_steps).astype(dtype = theano.config.floatX)],
                     outputs_info=[s_t0, pos_t0, v_t0, None, None, None, None, None, None, None, None, None, None, None, None, None, None],
...
FEa_mean_perturbations = FEta_th.mean(axis = 0).mean(axis = 1)
...
for i in range(len(action_params)):
    ...
    print 'Calculating derivative'
    normalization = T.nnet.softplus( action_sigmas[i] ) + sig_min_perturbations
    delta = T.tensordot(FEa_mean_perturbations,r_action_epsilons[i],axes = [[0],[0]])/normalization/n_perturbations   
        
    # USE ADAM OPTIMIZER
    p_adam = Adam(delta, action_params[i], 0.9, 0.999, learning_rate, epsilon = 10e-6)
    updates = updates + p_adam.updates
  
for i in range(len(action_sigmas)):
    
    print 'Creating updates for std dev of parameter %d...' % i
    
    print 'Calculating derivative'
    normalization = T.nnet.softplus( action_sigmas[i] ) + sig_min_perturbations
    outer_der = (r_action_epsilons[i]*r_action_epsilons[i]-1.0)/normalization
    inner_der = T.exp(action_sigmas[i])/(1.0 + T.exp(action_sigmas[i]))
    delta_sigma = T.tensordot(FEa_mean_perturbations,outer_der*inner_der,axes = [[0],[0]])/n_perturbations
 
    # USE ADAM OPTIMIZER
    p_adam = Adam(delta_sigma, action_sigmas[i], 0.9, 0.999, learning_rate, epsilon = 10e-6)
    updates = updates + p_adam.updates
...
params_save = params + action_params
sigmas_save = sigmas + action_sigmas
...
for i in range(n_steps):
    ...
    if i % saving_steps == 0:
        save_model(params_save, sigmas_save, base_name + '_%d.pkl' % i)
        save_model(params_save, sigmas_save, base_name + '_current.pkl')
            
    # Save best parameters
    if oFE_mean < FE_min:
        FE_min = oFE_mean
        save_model(params_save, sigmas_save, base_name + '_best.pkl')
        if save_best_trajectory == True:
            save_model(params_save, sigmas_save, base_name + '_best_%d.pkl' % i)
        

# Save final parameters
save_model(params_save, sigmas_save, base_name + '_final.pkl')
