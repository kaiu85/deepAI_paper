...
filename = 'supplement_no_proprioception_best_29797.pkl'
...
[Wq_hst_ot, Wq_hst_oht, Wq_hst_stm1, bq_hst,\
Wq_hst2_hst, bq_hst2,\
...
def inner_fn(t, stm1, postm1, vtm1):
    ...
    hst =  T.nnet.relu( T.dot(Wq_hst_stm1,T.reshape(stm1,(n_s,n_proc))) + T.dot(Wq_hst_ot,T.reshape(ot,(n_o,n_proc))) + T.dot(Wq_hst_oht,T.reshape(oht,(n_oh,n_proc))) + bq_hst )
...
fig_propagated.subplots_adjust(left=0.1, bottom=0.1, right=0.95, top=0.95,
                    wspace=0.3, hspace=0.3)
                    
fig_propagated.savefig('fig_no_proprioception_propagated.pdf')
...
free_energy = theano.function([], [FE_mean, KL_st_mean, p_ot_mean, p_oht_mean], allow_input_downcast = True, on_unused_input='ignore')
...
def inner_fn_sample(stm1):
    ...
    return st, ohtmu, ohtsig, ot, oht, prior_stmu, prior_stsig
...
((states_sampled, reward_probabilities_mu_sampled, reward_probabilities_sig_sampled, observations_sampled, rewards_sampled, stmus_sampled, stsigs_sampled), updates_sampling) =\
                     theano.scan(fn=inner_fn_sample,
                     outputs_info=[s_t0_sample, None, None, None, None, None, None],
...
# Build Function
eval_Penalized_FE = theano.function([], [FE_mean, observations_sampled, rewards_sampled, states_sampled], allow_input_downcast = True, on_unused_input='ignore')
...
plt.subplot(2,2,1)
plt.title('$o_a(t)$')
plt.text(0.5, 0.5, 'NOT PART OF\nGENERATIVE MODEL',horizontalalignment='center',verticalalignment='center')
#for j in range(10):
#    plt.plot(actions[:,0,j].squeeze())
#plt.xlabel('t / Steps')
#plt.ylabel('$o_a$ / a.u.')
...
fig_sampled.subplots_adjust(left=0.1, bottom=0.1, right=0.95, top=0.95,
                    wspace=0.3, hspace=0.3)
fig_sampled.savefig('fig_no_proprioception_sampled.pdf')
